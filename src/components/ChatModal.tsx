    ...
    try {
      ...
      if (reader) {
        ...
        if (!accumulatedText.trim()) {
          throw new Error('O limite de uso da API foi atingido temporariamente. Por favor, aguarde alguns segundos e tente novamente.');
        }
      }
    } catch (error) {
      ...
      setMessages(prev => [
        ...prev,
        {
          ...
          content: errorMsg.includes('API') || errorMsg.includes('Chave') || errorMsg.includes('limite')
            ? `⚠️ ${errorMsg}` 
            : 'Desculpe, tive um probleminha. 😅 Pode tentar enviar de novo daqui a pouco?',
        },
      ]);
    } finally {
      setIsLoading(false);
